...
class RecommendationService:
            ...
            try:
                if not os.path.exists(PARQUET_PATH):
                    raise FileNotFoundError(
                        f"Artifacts not found at {PARQUET_PATH}. Run: python build_index.py"
                    )

                print(f"[RecommendationService] Loading parquet from: {PARQUET_PATH}")
                self.df = pd.read_parquet(PARQUET_PATH)

                print("[RecommendationService] Initializing hybrid retriever...")
                hybrid_retriever.initialize()

                self.initialized = True
                print("[RecommendationService] Initialization complete.")

            except Exception as e:
                self.loading_error = str(e)
                print(f"[RecommendationService] Initialization failed: {e}")
                raise
    ...
    def start_background_initialize(self):
        """
        Starts initialization in a background thread so app startup is not blocked.
        """
        if self.initialized or self.loading:
            return
        ...
        def _target():
            try:
                ...
                # Error already stored in self.loading_error
                pass
    ...
    def _generate_reason(self, row, query: str) -> str:
        ...
        if any(word in q for word in ["study", "coding", "focus"]) and self._safe_float(
            row.get("Good for Work/Study", 0)
        ) > 0:
            reasons.append("strong work/study context match")

        if any(word in q for word in ["relax", "calm"]) and self._safe_float(
            row.get("Good for Relaxation/Meditation", 0)
        ) > 0:
            reasons.append("fits relaxation intent")

        if any(word in q for word in ["workout", "gym", "exercise"]) and self._safe_float(
            row.get("Energy", 0)
        ) > 0.6:
    ...
    def recommend(self, query: str, top_k: int = 5):
        if not self.initialized:
            raise RuntimeError("Recommendation service is not initialized yet.")
        ...
        candidates["reason"] = candidates.apply(
            lambda row: self._generate_reason(row, query), axis=1
        )
        candidates["rule_score"] = candidates.apply(
            lambda row: self._score_row(row, query), axis=1
        )
        candidates["final_score"] = (
            candidates["rrf_score"] * 3.0 + candidates["rule_score"] * 1.5
        )
